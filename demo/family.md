---
name: Family
description: Household, kids, and family life
icon: "\U0001F3E0"
tags:
    - family
    - home
list-collapse:
    - false
    - false
    - false
    - false
---

## Urgent

- [ ] Sign permission slip for field trip
  assignee: Dad
  priority: high
  due: 2026-03-21
  Emma's science museum trip on Friday

- [ ] Pay water bill
  priority: high
  due: 2026-03-25

- [ ] Schedule pediatrician checkup for Liam
  tags: health
  priority: medium
  due: 2026-03-28
  Annual wellness visit, bring vaccination records

- [ ] Fix leaking kitchen faucet
  tags: house
  priority: medium
  Getting worse, check washer first before calling plumber

## This Week

- [ ] Grocery run
  tags: errands
  Milk, eggs, chicken thighs, broccoli, snack bars for lunches

- [ ] Mow the lawn
  tags: house
  assignee: Dad

- [ ] Order birthday gift for Emma
  tags: kids
  due: 2026-04-05
  She wants the LEGO botanical set

- [ ] Prep meal plans for next week
  tags: cooking
  assignee: Dad
  Try that slow cooker pulled pork recipe

- [ ] Drop off donation bags at Goodwill
  tags: errands
  3 bags in the garage

## Upcoming

- [ ] Book summer camp registration
  tags: kids
  priority: high
  due: 2026-04-15
  YMCA fills up fast, register both kids

- [ ] Get car inspected
  tags: errands
  due: 2026-04-30
  Sticker expires end of April

- [ ] Plan family camping trip
  tags: family
  Memorial Day weekend, check state park availability

- [ ] Replace smoke detector batteries
  tags: house
  All 6 detectors, buy 9V bulk pack

- [ ] Research 529 plan options
  tags: finance
  Compare Vanguard vs Fidelity for college savings

## Done

- [x] File taxes
  tags: finance

- [x] Set up kids' chore chart
  tags: kids

- [x] Replace furnace filter
  tags: house

- [x] Parent-teacher conference
  tags: kids
  assignee: Dad
